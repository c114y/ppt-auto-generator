...
def build_ppt_file(outline, template_path="template.pptx"):
    """基于模板自动化排版并生成 PPTX 文件流（含动态防崩溃机制）"""
    try:
    ...
    for page in pages:
        ...
        # 核心修复：动态寻找内容占位符，若找不到则自动绘制文本框（彻底解决 idx == 1 报错）
        body_shape = None
        for shape in slide.placeholders:
            # 排除标题(idx 0)，寻找任何可用的内容占位符
            if shape.is_placeholder and shape.placeholder_format.idx != 0:
                body_shape = shape
                break
        
        # 如果找到了官方占位符，清空里面的提示文字
        if body_shape:
            tf = body_shape.text_frame
            tf.clear() 
        else:
            # 后备方案：如果没有找到占位符，代码自动画一个文本框
            from pptx.util import Inches
            left = Inches(1.0)
            top = Inches(1.8) # 避开标题区域
            width = Inches(8.0)
            height = Inches(5.0)
